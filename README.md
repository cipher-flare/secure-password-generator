...
├── requirements.txt
├── _verify_core.py              # One-off test harness for password_core.py
├── templates/
│   ├── index.html               # Single-page UI
│   └── _icons_sprite.html       # SVG icon sprite (included by index.html)
├── static/
│   ├── css/
...
│   │   ├── strength.js          # Entropy, NIST score, zxcvbn-lite auditor
│   │   ├── history.js           # localStorage CRUD + JSON import/export
│   │   └── ui.js                # DOM wiring, toasts, modal, clipboard
│   └── img/
│       └── favicon.svg
└── archive/
...
`_verify_core.py` is a one-off test harness (gitignored) that exercises validation, the category guarantee across many length/category combinations, the entropy formula, crack-time monotonicity, the strength classifier, and the shape of `password_stats`. It exits non-zero on any failure.

```bash
python _verify_core.py
```
...
- `sha1Hex(str)` — SHA-1 via `crypto.subtle.digest` (used by the "Check SHA-1 prefix" button; the actual range lookup against HIBP is stubbed).

## History (`static/js/history.js`)

Stored in `localStorage` under `spg.history.v1`. Capped at 200 entries. De-dupes by password value. `importJSON` merges by password and ignores entries without a non-empty string `password`.
...
- **No external requests**: CSP locks `default-src`, `script-src`, `style-src`, `img-src`, `font-src`, and `connect-src` to `'self'`. The HIBP range lookup is intentionally stubbed and would need to be enabled with an explicit `connect-src` exception.
